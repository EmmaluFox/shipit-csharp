...
namespace ShipIt.Parsers
{
    public static class ProductParser
    {
        public static Product Parse(this ProductRequestModel requestModel)
        {
            ...
            if (string.IsNullOrEmpty(requestModel.Discontinued)) errors.Add("Discontinued must be set");
            if (string.IsNullOrEmpty(requestModel.LowerThreshold)) errors.Add("LowerThreshold must be set");
            if (string.IsNullOrEmpty(requestModel.MinimumOrderQuantity)) errors.Add("MinimumOrderQuantity must be set");
            if (string.IsNullOrEmpty(requestModel.Weight)) errors.Add("Weight must be set");

            if (errors.Any()) throw new MalformedRequestException(string.Join("\n", errors));
            ...
            if (!int.TryParse(requestModel.Weight, out weight)) errors.Add("Weight must be set to an integer");
